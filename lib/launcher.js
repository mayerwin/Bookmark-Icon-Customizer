 ...
 *   the BOOKMARKLET_DATA_MARKER in the URL, extracts the source from the
 *   meta tag, and chrome.scripting.executeScript's it into the now-active
 *   tab's MAIN world. Same execution surface a native javascript: URL has.
 *   Requires <all_urls> host permission (requested by the popup).
 *
 * The original JS source is stored URL-encoded in a meta tag so the popup
 * can recover it (restore original, toggle mode, change icon).
 */
